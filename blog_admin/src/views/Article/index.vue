  ...
  <div class="pagebox">
    ...
    <div class="pagebox-content">
      <el-table :data="tableData" border size="mini" v-loading="!ableToLoad" stripe>
        ...
        <el-table-column prop="browser" label="浏览量" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column label="操作" width="350">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              @click.native.prevent="isDel(false, scope.row)"
              type="primary"
              size="mini"
              >上架</el-button
            >
            <el-button v-if="!scope.row.is_del" @click.native.prevent="isDel(true, scope.row)" type="danger" size="mini"
              >下架</el-button
            >
            <el-button @click.native.prevent="comment(scope.row)" type="info" size="mini">评论</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    ...
    comment(row) {
      this.$router.push(`/article/comment/${row.id}`)
    },
    ...
    async isDel(is_del, item) {
      await this.$api.article.hiddenArticle(item.id, { is_del })
      this.$message({
        type: 'success',
    ...
    async delHandler(item) {
      await this.$api.article.deleteArticle(item.id)
      this.$message({
        type: 'success',
        message: '删除成功',
      })
    ...
    async getList() {
      ...
      const res = await this.$api.article.getArticleList(params)
